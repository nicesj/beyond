...
#ifndef __BEYOND_INTERNAL_INFERENCE_IMPL_LOCAL_H__
#define __BEYOND_INTERNAL_INFERENCE_IMPL_LOCAL_H__
...
class Inference::impl::local final : public Inference::impl {
public: // Inference interface
    static local *Create(bool autoSplit = false);
...
private:
    ...
    // TODO:
    // only one runtime until we design a policy (or algorithm) for multiple runtimes
    InferenceInterface::RuntimeInterface *runtime;

private:
    local(void);
    ~local(void) = default;

    static beyond_handler_return RuntimeEventHandler(beyond_object_h obj, int type, beyond_event_info *eventInfo, void *data);
};

} // namespace beyond

#endif // __BEYOND_INTERNAL_INFERENCE_IMPL_LOCAL_H__
